...
Environment specific configurations can be done in the `.env` file. It will be located in the root directory of the Sentry installation.

By default, there exists no `.env` file. In this case, you can either manually add this file by copying the `.env.example` file to a new `.env` file or you simply follow the below **Installation**, which will automatically create the missing `.env` file, if there is none yet.

If there already exists a `.env` file, the installer will not touch it. So please keep in mind to check the `.env.example` file for changes, when you perform an upgrade of Sentry.
